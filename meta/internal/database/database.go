...
//
// SQLite uses separate write and read pools: the write pool is limited to one
// connection for SQLite's single-writer model, while the read pool allows
// concurrent read queries through WAL mode. Postgres uses the same GORM handle
...
func newSQLite(dsn string, dbLogger logger.Interface) (*DB, error) {
	...
	writeDB, err := gorm.Open(sqlite.Open(appendParams(baseDSN, writeParams)), &gorm.Config{Logger: dbLogger})
	if err != nil {
	...
	readDB, err := gorm.Open(sqlite.Open(appendParams(baseDSN, readParams)), &gorm.Config{Logger: dbLogger})
	if err != nil {
...
func (db *DB) Migrate() error {
	log.Println("Running Meta GORM AutoMigrate...")
