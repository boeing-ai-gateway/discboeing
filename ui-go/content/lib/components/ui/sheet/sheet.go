...
func closeClass(className string) string {
	return classnames.CN("ring-offset-background focus-visible:ring-ring absolute end-4 top-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-hidden disabled:pointer-events-none", className)
}
...
func footerClass(className string) string {
	return classnames.CN("mt-auto flex flex-col gap-2 p-4", className)
}

func headerClass(className string) string {
	return classnames.CN("flex flex-col gap-1.5 p-4", className)
}

func titleClass(className string) string {
	return classnames.CN("text-foreground font-semibold", className)
}
