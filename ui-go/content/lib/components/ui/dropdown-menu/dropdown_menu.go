...
func labelClass(className string) string {
	return classnames.CN("px-2 py-1.5 text-sm font-semibold data-[inset]:ps-8", className)
}

func separatorClass(className string) string {
	return classnames.CN("bg-border -mx-1 my-1 h-px", className)
}

func shortcutClass(className string) string {
	return classnames.CN("text-muted-foreground ms-auto text-xs tracking-widest", className)
}
