...
func buttonGroupSeparatorClass(className string) string {
	return joinClass("bg-input relative !m-0 self-stretch data-[orientation=vertical]:h-auto", className)
}

func buttonGroupTextClass(className string) string {
	return joinClass("bg-muted flex items-center gap-2 rounded-md border px-4 text-sm font-medium shadow-xs [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4", className)
}
