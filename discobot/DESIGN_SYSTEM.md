...
The workspace uses a three-pane IDE structure:
...
- Center pane: primary session canvas and composer.
...
| `--workspace-gap` | Gap between center and right panes |
| `--workspace-panel-radius` | Major pane radius |
...
- Major pane padding: `10px` to `12px`.
- Pane gap: `10px`.
...
- Single-use component structure may keep its CSS inline in the owning templ
  ...
  `app-shell__window`, so their ownership is obvious even though CSS selectors
  remain global.
...
calls. If a new VS Code product icon is needed, add its Codicon name to
`scripts/build-codicon-sprite.mjs`, create a typed helper, and use that helper
from templates.

`pnpm assets:build` generates `static/vendor/codicons.svg` from the selected
`@vscode/codicons` SVG files. Codicons are provided by Microsoft under the
Creative Commons Attribution 4.0 International license; keep the package
attribution when changing the icon pipeline.
