...
test("createEntityStore composes createResource for list and per-item SWR state", () => {
	...
	assert.match(
		source,
		/syncItemResourcesFromList\(mergedItems, \{[\s\S]*markFresh: true,[\s\S]*clearMissing: true,[\s\S]*freshAt: startedAt,[\s\S]*\}\);/,
	);
});
...
test("createEntityStore keeps indexed item resources synchronized with collection updates", () => {
	...
	assert.match(
		source,
		/itemResources\.get\(id\)\?\.setData\(item, \{[\s\S]*markFresh: options\.markFresh,[\s\S]*freshAt: options\.freshAt,[\s\S]*\}\);/,
	);
...
test("createEntityStore builds per-item resources with indexed and list-backed loading paths", () => {
	...
	assert.match(
		source,
		/if \(args\.indexed\?\.isNotFoundError\?\.\(error\)\) \{[\s\S]*if \(args\.indexed\.notFound === "evict"\) \{[\s\S]*evictInternal\(id, \{ markFresh: true, freshAt: startedAt \}\);[\s\S]*\} else \{[\s\S]*resource\.setData\(null, \{[\s\S]*markFresh: true,[\s\S]*freshAt: startedAt,[\s\S]*\}\);/,
	);
	assert.match(
		source,
