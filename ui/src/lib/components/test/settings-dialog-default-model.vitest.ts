...
const source = readFileSync(
	path.resolve(import.meta.dirname, "../app/SettingsDialog.svelte"),
	"utf8",
);
const uiStateSource = readFileSync(
	path.resolve(import.meta.dirname, "../../context/stores/preferences.ts"),
	"utf8",
);
...
test("settings dialog preserves the selected default model when dedupe would hide it", () => {
	...
	assert.match(
		source,
		/models\.find\(\(model\) => model\.id === preferences\.defaultModel\)/,
	);
	assert.match(
		source,
