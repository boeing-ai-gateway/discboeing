...
test("conversation composer passes pending session state to the submit button", () => {
	...
	assert.match(source, /<ConversationComposerSubmitButton[\s\S]*\{isPending\}/);
});
...
test("conversation composer disables input when no models are available", () => {
	...
	assert.match(
		source,
		/onclick=\{\(\) =>\s*void context\.commands\.dialogs\.openCredentialsDialog\(\)\}/,
	);
	assert.match(
		source,
...
test("conversation composer keeps session credentials interactive during pending questions", () => {
	...
	assert.match(source, /<ConversationCredentialsControl \{sessionId\} \/>/);
	assert.doesNotMatch(
		source,
