...
test("session setup status shows creation while pending chat is starting", () => {
	...
	assert.match(source, /context\.data\.sessions\.byId\[sessionId\]/);
	assert.match(source, /context\.view\.sessions\[sessionId\]/);
	assert.doesNotMatch(source, /SessionRuntimeState/);
	assert.doesNotMatch(source, /getThreadRuntimeSnapshot/);
	assert.doesNotMatch(source, /legacy-context-bridge/);
	assert.doesNotMatch(source, /useThreadBridge/);
	assert.doesNotMatch(source, /ThreadRuntimeController/);
	assert.match(
		source,
