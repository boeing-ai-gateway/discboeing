...
test("app keyboard shortcuts owns the global keyboard controller", () => {
	...
	assert.match(
		source,
		/import \{[\s\S]*getGlobalShortcuts,[\s\S]*matchGlobalShortcutKeydown,[\s\S]*shouldCommitTabSwitcherOnKeyup,[\s\S]*\} from "\$lib\/shortcuts\/global-shortcuts";/,
	);
	assert.doesNotMatch(source, /detectIsMacPlatform/);
	assert.match(
		source,
		/import \{[\s\S]*getAvailableSwitcherThreads,[\s\S]*getThreadSwitcherThreads,[\s\S]*recentThreadKey,[\s\S]*\} from "\$lib\/context\/view\/thread-switcher";/,
	);
	...
	assert.match(source, /sessions: sessionItems,/);
	assert.match(
		source,
	...
	assert.match(
		source,
		/context\.commands\.dialogs\.setRecentThreadSwitcherOpen/,
	);
	assert.match(
		source,
		/context\.commands\.dialogs\.toggleKeyboardShortcutsOpen/,
	);
	assert.match(
		source,
		/context\.commands\.navigation\.setMobileSidebarOpen\(false\);/,
	);
	assert.match(source, /appEnvironment\.isMobile/);
	assert.match(source, /context\.view\.selection\.sessionId/);
	assert.match(
		source,
		/context\.commands\.navigation\.openThread\(\s*selectedThread\.sessionId,\s*selectedThread\.threadId,/,
	);
	assert.match(source, /context\.commands\.navigation\.startNewSession\(\);/);
	assert.match(
		source,
		/context\.commands\.threads\s*\.\s*createThread\(sessionId,/,
	);
