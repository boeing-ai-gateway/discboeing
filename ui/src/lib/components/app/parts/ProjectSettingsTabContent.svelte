<script lang="ts">
	...
	async function loadResources(force = false) {
		...
		resourcesError = null;
		try {
			const nextResources = await api.getProjectResources();
	...
	async function loadInspection(force = false) {
		...
		try {
			inspection = await api.getProjectInspection();
	...
	async function refreshSystemSettings() {
		saveSuccess = null;
		await Promise.all([loadResources(true), loadInspection(true)]);
	}
	...
	async function handleSave() {
		...
		try {
			...
			const result = await api.updateProjectResources(payload);
				...
				<ItemGroup class="rounded-md border border-border">
					<Item size="sm">
						<ItemContent>
							...
							<ItemDescription>
								{resources.provider} · {resources.vm.cpuCount} CPU{resources.vm
									.cpuCount === 1
									? ""
									: "s"}
							</ItemDescription>
					...
					<Item size="sm">
						...
						<ItemActions class="ml-auto w-40 justify-end">
							...
							<Input
								...
								value={memoryDraft}
								oninput={(event) => {
									memoryDraft = (event.currentTarget as HTMLInputElement).value;
									saveError = null;
									saveSuccess = null;
								}}
								class="text-right"
...
<ProjectInspectionTerminalDialog
	...
	description="Troubleshooting shell for the inspection container."
/>
