	...
	if (href && options.onLinkClick) {
		...
		if (isIncomplete) {
			...
			button.addEventListener("click", (event) => {
				...
				const shouldContinue = options.onLinkClick?.({
					href,
...
function createCopyButton(code: string): HTMLButtonElement {
	const button = document.createElement("button");
	button.type = "button";
	button.dataset.streamdown = "copy-button";
...
function renderHighlightedCodeBody(
	...
	for (const row of result.tokens) {
		...
		for (const token of row) {
			...
			tokenElement.dataset.streamdown = "code-token";
			if (token.color) {
...
function renderImage(
	...
	overlay.dataset.streamdown = "image-overlay";
	...
	downloadButton.addEventListener("click", () => {
		const shouldContinue = options.onImageDownload?.({
			alt,
			src,
			suggestedFilename: createImageFilename(src, alt),
		});
		if (shouldContinue === false) {
			return;
		}
...
function renderCodeBlock(
	...
	if (language) {
		...
		title.dataset.streamdown = "code-block-title";
		title.textContent = language;
		header.append(title);
	}

	const actionsRow = document.createElement("div");
	actionsRow.dataset.streamdown = "code-block-actions-row";
	...
	bodyContainer.dataset.streamdown = "code-block-body-container";
...
function renderElement(
	...
	if (
		...
		return renderImage(node.children[0], options);
	}
