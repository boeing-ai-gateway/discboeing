...
function quoteCommentSnippet(snippet: string): string {
	return snippet
		.split(/\r?\n/)
		.map((line) => `> ${line}`)
		.join("\n");
...
function getDynamicToolParts(message: ChatMessage): DynamicToolPart[] {
...
export function getHookDisplayState(
	hook: Pick<HookRunStatus, "hookId" | "lastResult">,
