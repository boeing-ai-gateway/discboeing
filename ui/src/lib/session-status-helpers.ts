import { CommitOperation, CommitStatus } from "$lib/api-constants";
import type {
	Session,
...
export function resolveSessionDisplayStatus(
	...
	if (sessionActivityStatus && sessionActivityStatus !== "idle") {
		return sessionActivityStatus as SessionDisplayStatus;
	}
	return (
		sessionStatus === "ready" || !sessionStatus ? "idle" : sessionStatus
	) as SessionDisplayStatus;
}
...
export function resolveSidebarThreadStatus({
	...
	if (sessionActivityStatus && sessionActivityStatus !== "idle") {
		return sessionActivityStatus as ThreadActivityStatus["status"];
	}
