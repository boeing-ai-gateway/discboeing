...
func TestEvents_SSE_Connection(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: "discboeing_session", Value: user.Token})
...
func TestEvents_SessionCreationEmitsEvents(t *testing.T) {
	...
	createResp := client.Post(threadChatPath(project.ID, "test-events-session-1", "test-events-session-1"), map[string]any{
