...
func TestLoadStaticTLS(t *testing.T) {
	...
	if setup == nil || len(setup.TLSConfig.Certificates) != 1 {
		t.Fatalf("expected static certificate to load")
	}
}
...
func TestRedirectHTTPToHTTPS(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "http://localhost:3001/api/status?x=1", nil)
	...
	if location := recorder.Header().Get("Location"); location != "https://localhost:3443/api/status?x=1" {
...
func newTestStore(t *testing.T) *store.Store {
	...
	return store.New(db.DB, db.ReadDB)
}
