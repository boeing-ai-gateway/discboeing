...
DEFAULT_USER=discboeing
PROXY_BINARY=/opt/discboeing/bin/proxy
PROXY_PORT=17080
DATA_DIR=/.data
BASE_HOME_DIR=/.data/discboeing
WORKSPACE_DIR=/.data/discboeing/workspace
STAGING_DIR=/.data/discboeing/workspace.staging
OVERLAYFS_DIR=/.data/.overlayfs
MOUNT_HOME=/home/discboeing

log() {
	printf 'discboeing-sandbox-init: %s\n' "$*"
}
...
for line in lines:
    ...
    if ip == '127.0.0.1':
        ...
        modified = True
        print("discboeing-sandbox-init: removed 'localhost' from ::1 line in /etc/hosts")
    else:
        out.append(line)
if not has_ipv4:
    out.insert(0, '127.0.0.1\tlocalhost')
    modified = True
    print("discboeing-sandbox-init: added '127.0.0.1 localhost' to /etc/hosts")
if modified:
    path.write_text('\n'.join(out) + '\n')
    print('discboeing-sandbox-init: /etc/hosts updated to ensure localhost resolves to 127.0.0.1')
else:
    print('discboeing-sandbox-init: /etc/hosts already configured correctly for localhost')
PY
}
...
remove_obsolete_bundled_home_config() {
	local home=$1 rel
	for rel in \
		.discboeing/scripts/discboeing-commit \
		.discboeing/scripts/discboeing-commit-remote \
		.discboeing/scripts/discboeing-rebase \
		.discboeing/commands/discboeing-commit.md \
		.discboeing/commands/discboeing-commit-remote.md \
		.discboeing/commands/discboeing-rebase.md \
		.discboeing/skills/browser-harness/SKILL.md
	do
		rm -rf "${home:?}/$rel"
	done
...
write_default_proxy_config() {
	cat <<'EOF'
# Default Discboeing Proxy Configuration
# Written by discboeing-sandbox-init during sandbox startup.
...
set_proxy_in_profile() {
	local profile_path=/etc/profile.d/discboeing-proxy.sh
	mkdir -p /etc/profile.d
	{
		printf '%s\n' '# Discboeing Proxy Configuration'
		printf '%s\n' '# Automatically generated by discboeing-sandbox-init'
...
write_proxy_environment_file() {
	...
	mkdir -p /run/discboeing
	proxy_env_lines > /run/discboeing/proxy-env
	chmod 0644 /run/discboeing/proxy-env
	log 'proxy environment written to /run/discboeing/proxy-env'
...
write_agent_environment_file() {
	...
	mkdir -p /run/discboeing
	{
		...
		printf 'USER=%s\n' "$user"
		printf 'LOGNAME=%s\n' "$user"
		printf 'DISCBOEING_HOOKS_ENABLED=true\n'
		proxy_env_lines
	} > /run/discboeing/agent-env
	chmod 0644 /run/discboeing/agent-env
	log 'agent environment written to /run/discboeing/agent-env'
}
...
cache_paths() {
	cat <<'EOF'
/home/discboeing/.cache
/home/discboeing/.npm
/home/discboeing/.pnpm-store
/home/discboeing/.yarn
/home/discboeing/.local/share/uv
/home/discboeing/go/pkg/mod
/home/discboeing/.cargo/registry
/home/discboeing/.cargo/git
/home/discboeing/.rustup
/home/discboeing/.bundle
/home/discboeing/.gem
/home/discboeing/.m2/repository
/home/discboeing/.gradle/caches
/home/discboeing/.gradle/wrapper
/home/discboeing/.nuget/packages
/home/discboeing/.composer/cache
/nix
/var/cache/apt
/home/discboeing/.ccache
/home/discboeing/.config/JetBrains
/home/discboeing/.local/share/JetBrains/Toolbox/apps
/home/discboeing/.local/share/JetBrains/Daemon/bundles
/home/discboeing/.local/share/discboeing-code-server/Machine
/home/discboeing/.local/share/discboeing-code-server/extensions
/home/discboeing/.vscode-server
/home/discboeing/.cursor-server
/home/discboeing/.zed_server
EOF
	local cfg="$MOUNT_HOME/workspace/.discboeing/cache.json"
	if [[ -f $cfg ]]; then
		jq -r '.additionalPaths[]? // empty' "$cfg" 2>/dev/null | while IFS= read -r path; do
			case "$(realpath -m "$path")" in
				/home/discboeing/*) printf '%s\n' "$(realpath -m "$path")" ;;
...
mount_cache_directories() {
	...
	while IFS= read -r cache_path; do
		...
		target_root=/home/discboeing
		if [[ $cache_path != /home/discboeing/* ]]; then
...
run_setup() {
	...
	session_id=${DISCBOEING_SESSION_ID:-}
	if [[ -z $session_id ]]; then
		printf 'DISCBOEING_SESSION_ID environment variable is required\n' >&2
		return 1
	fi
	require_user "$run_as_user"

	step_start=$(date +%s%3N); setup_git_safe_directories; step_end=$(date +%s%3N); elapsed=$((step_end - step_start)); printf 'discboeing-sandbox-init: [%d.%03ds] git safe.directory setup completed\n' "$((elapsed / 1000))" "$((elapsed % 1000))"
	step_start=$(date +%s%3N); setup_base_home "$run_as_user"; remove_obsolete_bundled_home_config "$BASE_HOME_DIR"; step_end=$(date +%s%3N); elapsed=$((step_end - step_start)); printf 'discboeing-sandbox-init: [%d.%03ds] base home setup completed\n' "$((elapsed / 1000))" "$((elapsed % 1000))"
	...
	step_end=$(date +%s%3N); elapsed=$((step_end - step_start)); printf 'discboeing-sandbox-init: [%d.%03ds] filesystem setup completed (overlayfs)\n' "$((elapsed / 1000))" "$((elapsed % 1000))"

	step_start=$(date +%s%3N); mount_cache_directories || log 'Cache mount failed'; step_end=$(date +%s%3N); elapsed=$((step_end - step_start)); printf 'discboeing-sandbox-init: [%d.%03ds] cache directories mounted\n' "$((elapsed / 1000))" "$((elapsed % 1000))"
	step_start=$(date +%s%3N); setup_proxy_config || log 'Proxy config setup failed'; step_end=$(date +%s%3N); elapsed=$((step_end - step_start)); printf 'discboeing-sandbox-init: [%d.%03ds] proxy config setup completed\n' "$((elapsed / 1000))" "$((elapsed % 1000))"
	step_start=$(date +%s%3N); setup_proxy_certificate "$run_as_user" || log 'Proxy certificate setup failed'; step_end=$(date +%s%3N); elapsed=$((step_end - step_start)); printf 'discboeing-sandbox-init: [%d.%03ds] proxy certificate setup completed\n' "$((elapsed / 1000))" "$((elapsed % 1000))"
	step_start=$(date +%s%3N); write_docker_daemon_config || log 'Docker daemon config failed'; step_end=$(date +%s%3N); elapsed=$((step_end - step_start)); printf 'discboeing-sandbox-init: [%d.%03ds] Docker daemon config written\n' "$((elapsed / 1000))" "$((elapsed % 1000))"

	rm -f "$(user_home "$run_as_user")/.docker/buildx/current" "$(user_home "$run_as_user")/.docker/buildx/instances/discboeing-shared"

	step_start=$(date +%s%3N)
	write_proxy_environment_file || warn 'failed to write proxy env file'
	write_agent_environment_file "$run_as_user"
	step_end=$(date +%s%3N); elapsed=$((step_end - step_start)); printf 'discboeing-sandbox-init: [%d.%03ds] environment files written\n' "$((elapsed / 1000))" "$((elapsed % 1000))"

	step_end=$(date +%s%3N); elapsed=$((step_end - startup_start)); printf 'discboeing-sandbox-init: [%d.%03ds] setup completed successfully\n' "$((elapsed / 1000))" "$((elapsed % 1000))"
	notify_ready
}

main() {
	if [[ $# -lt 1 || $1 != setup ]]; then
		printf 'usage: discboeing-sandbox-init <setup>\n' >&2
		return 1
	fi
	run_setup
}
