...
AGENT_ENV_FILE="${DISCOBOT_AGENT_ENV_FILE:-/run/discobot/agent-env}"
WORKSPACE_ENV_FILE="${DISCOBOT_WORKSPACE_ENV_FILE:-/workspace/.discobot/env}"
CR=$(printf '\r')

warn_invalid_env_line() {
	env_file=$1
	line_number=$2
	echo "discobot-session-env: warning: ignoring invalid env line ${line_number} in ${env_file}" >&2
}

warn_unreadable_env_file() {
	env_file=$1
	echo "discobot-session-env: warning: cannot read env file ${env_file}" >&2
}
...
if [ "$#" -eq 0 ]; then
	echo "discobot-session-env: command is required" >&2
	exit 2
fi
