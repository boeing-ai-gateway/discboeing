...
func TestAppendAndLoadBrowserEvents(t *testing.T) {
	store := NewStore(t.TempDir())

	if err := store.AppendBrowserEvent("thread1", "turn1", 2, BrowserEvent{
		RequestID: "7",
...
func TestSaveBrowserScreenshot(t *testing.T) {
	store := NewStore(t.TempDir())
...
func TestSaveBrowserScreenshot_DeduplicatesByContent(t *testing.T) {
	store := NewStore(t.TempDir())
	png := []byte("\x89PNG\r\n\x1a\nsame")
...
func TestLoadAllBrowserEventEntries(t *testing.T) {
	store := NewStore(t.TempDir())
	...
	if err := store.AppendBrowserEvent("thread1", "turn-a", 0, BrowserEvent{
		EventID:   "browser-1",
		Method:    "Page.navigate",
		Direction: "request",
	}); err != nil {
