...
// InstallSystemScripts writes built-in Discboeing scripts into dir. The commit
...
func InstallSystemScripts(dir, workspaceSource string) error {
	...
	if err := os.Remove(filepath.Join(dir, "discboeing-commit-remote")); err != nil && !os.IsNotExist(err) {
...
// Scripts returns built-in Discboeing scripts from memory.
func Scripts(workspaceSource string) []ScriptFile {
	commitPath := "scripts/discboeing-commit"
	if isGitURL(workspaceSource) {
		commitPath = "scripts/discboeing-commit-remote"
	}

	var scripts []ScriptFile
	for _, path := range []string{commitPath, "scripts/discboeing-rebase"} {
		content, err := assets.ReadFile(path)
		if err != nil {
			continue
		}
		name := filepath.Base(path)
		if name == "discboeing-commit-remote" {
